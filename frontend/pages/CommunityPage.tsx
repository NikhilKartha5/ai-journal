...
import { fetchCommunityFeed, createCommunityPost, toggleLikeCommunityPost } from '../services/apiService';
...
const CommunityPage: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-6">Community Wellness Feed</h1>
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 mb-8 shadow-sm">
        ...
        <textarea
          className="w-full min-h-[90px] bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md p-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 resize-y"
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <button disabled={submitting || content.trim().length<5} onClick={handleSubmit} className="flex items-center bg-sky-500 hover:bg-sky-600 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-md shadow">
        ...
        {loading ? <p className="text-slate-500 dark:text-slate-400">Loading...</p> : (
          posts.length === 0 ? <p className="text-slate-500 dark:text-slate-400">No posts yet. Be the first to share something supportive.</p> : (
            <ul className="space-y-4">
              {posts.map(p => (
                <li key={p.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4 shadow-sm">
